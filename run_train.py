...
def main():
  for ticker in ['BTC_ETH_2h', 'BTC_DGB_2h']:
    for target in ['high']:
      ...
      job_runner = JobRunner(job_info, limit=np.min)
      job_runner.iterate(iterations=10, params_fun=lambda : {
        ...
        'model_params': {
          'batch_size': np.random.choice([1000, 2000, 3000, 4000]),
          'epochs': 100,
          'hidden_layer': np.random.randint(40, 500),
          'learning_rate': 10 ** np.random.uniform(-2.0, 0.5),
