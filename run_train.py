...
def main():
  for ticker in ['BTC_ETH_2h', 'BTC_DGB_2h']:
    for target in ['high']:
      ...
      job_runner = JobRunner(job_info, limit=0.2)
      job_runner.iterate(iterations=30, params_fun=lambda : {
        'target': job_info.target,
        'k': np.random.choice([1, 2, 3, 4, 5, 6, 7, 8]),
        'model_class': NeuralNetworkModel,
        'model_params': {
          'batch_size': np.random.choice([200, 500, 1000, 2000]),
          'epochs': 100,
